  ...
  try {
    ...
    if (!serializedState) return undefined;
      ...
      if (!("colorMap" in parsedState) || !parsedState.colorMap.length) {
        ...
        localStorage.setItem("classesInfo", JSON.stringify(parsedState));
      }
...
const classesSlice = createSlice({
  ...
  reducers: {
    ...
    changeModuleCodeLessonTypeOrder(state, action: PayloadAction<string[]>) {
      return {
        ...state,
    ...
    removeModule(state, action: PayloadAction<string>) {
      ...
      if (!stillExists) {
        Object.keys(newNonBiddable).forEach((mclt) => {
          if (mclt.startsWith(moduleCode + ": ")) {
    ...
    addAvailableClasses(state, action: PayloadAction<ModuleCodeLessonType>) {
      return {
        ...state,
        totalModuleCodeLessonTypeMap: {
          ...state.totalModuleCodeLessonTypeMap,
    ...
    changeClassOrder(
      ...
      return {
        ...
        selectedClasses: {
          ...state.selectedClasses,
          [action.payload.moduleCodeLessonType]: action.payload.newOrder,
        },
        lastUpdated: new Date().getTime(),
      };
    },
    ...
    updateMainList(state, action: PayloadAction<string>) {
      ...
      const newClassesSelected: ClassOverview[] = copiedAvailableClasses[
        moduleCodeLessonType
      ].filter((class_) => newClasses.includes(class_.classNo));
